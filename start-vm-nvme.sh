...
qemu-system-x86_64 \
        ...
        -name "${MACHINE_NAME}" \
	-drive file="${QEMU_IMG0}",if=none,id=nvm0 -device nvme,serial=deadbeef0,drive=nvm0 \
	-drive file="${QEMU_IMG1}",if=none,id=nvm1 -device nvme,serial=deadbeef1,drive=nvm1 \
